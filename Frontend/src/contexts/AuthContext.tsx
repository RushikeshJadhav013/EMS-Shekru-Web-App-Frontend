  ...
  const login = async (userData: LoginResponse) => {
    ...
    try {
      ...
      const user: User = {
        ...
        profilePhoto: userData.profile_photo ? `testing.staffly.space/${userData.profile_photo}` : undefined,
        status: 'active',
      ...
      try {
        await fetch('testing.staffly.space/attendance/login-resume', {
          method: 'POST',
          headers: {
  ...
  const logout = async (showToast = true) => {
    ...
    try {
      ...
      if (token && isTokenValid(token) && user?.id) {
        ...
        await fetch('testing.staffly.space/attendance/logout', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
