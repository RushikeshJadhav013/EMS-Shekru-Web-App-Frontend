...
const MeetingsPage: React.FC = () => {
  ...
  const normalizeMeeting = (m: any): Meeting => {
    ...
    const projectId =
      m.project_id || m.projectId || m.project?.id || m.project?.project_id;
    ...
    if (projectId || projectName) {
      ...
      if (rawType.includes("project")) {
        ...
        rawType.includes("townhall") ||
        rawType.includes("general") ||
        rawType.includes("company") ||
        rawType === "all"
      ) {
        rawType = "company";
      } else if (
        rawType.includes("1:1") ||
  ...
  const fetchData = async () => {
    try {
      ...
      const [allMeetingsData, myMeetingsData, depts, projs, emps] =
        await Promise.all([
      ...
      const normalizedAll = (
        Array.isArray(allMeetingsData) ? allMeetingsData : []
      ).map(normalizeMeeting);
      const normalizedMy = (
        Array.isArray(myMeetingsData) ? myMeetingsData : []
      ).map(normalizeMeeting);
      ...
      setProjects(
        Array.isArray(projs) ? projs : (projs as any)?.projects || [],
      );
  ...
  const handleCreateMeeting = async () => {
    ...
    try {
      ...
      if (selectedMeeting) {
        if (formData.type === "project" && formData.project_id) {
          await apiService.updateProjectMeeting(
            formData.project_id,
            selectedMeeting.id,
            payload,
          );
            ...
            <DialogContent className="sm:max-w-[650px] rounded-[2rem] border-none shadow-2xl p-0 overflow-hidden">
              <div className="bg-gradient-to-br from-blue-600 to-indigo-700 p-8 text-white">
                ...
                <DialogDescription className="text-blue-100 font-medium opacity-80 uppercase text-[10px] tracking-widest mt-1">
                  {selectedMeeting
                    ...
                    : "Create a new meeting with an existing Google Meet or Zoom URL."}
                </DialogDescription>
              </div>
                  ...
                  <div className="grid gap-2">
                    ...
                    <Select
                      ...
                      onValueChange={(v) =>
                        setFormData({
                          ...formData,
                          team_id: v ? parseInt(v) : undefined,
                        })
                      }
                    >
                        ...
                        departments.filter(Boolean).length > 0 ? (
                          departments.filter(Boolean).map((d: any) => (
                            <SelectItem
                              ...
                              value={d.id?.toString() || ""}
                            >
                              {d.name || d.code || "Unnamed Department"}
                            </SelectItem>
                          ))
                        ) : (
                    ...
                    <Select
                      ...
                      onValueChange={async (v) => {
                        ...
                        if (pId) {
                          try {
                            const members =
                              await apiService.getProjectMembers(pId);
                            const memberIds = (
                      ...
                      <SelectContent>
                        ...
                        projects.filter(Boolean).length > 0 ? (
                          projects.filter(Boolean).map((p: any) => (
                            <SelectItem
                              key={p.project_id || p.id || Math.random()}
                  ...
                  <div className="grid grid-cols-2 gap-3 max-h-[200px] overflow-y-auto p-1 custom-scrollbar">
                    {employees
                      ...
                      .map((emp: any) => {
                        ...
                        const isSelected =
                          formData.participant_ids.includes(empId);
                        return (
                          <div
                          ...
                          >
                            ...
                            <div className="h-8 w-8 rounded-xl bg-slate-100 dark:bg-slate-800 flex items-center justify-center overflow-hidden">
                              <img
                                src={`https://api.dicebear.com/7.x/avataaars/svg?seed=${emp.name || emp.id}`}
                                alt=""
              ...
              <DialogFooter className="p-8 bg-slate-50 dark:bg-slate-900/50 flex !justify-center gap-3 border-t border-slate-100 dark:border-slate-800">
                <Button
                  ...
                  className="rounded-xl h-11 px-6 font-bold text-xs uppercase tracking-widest text-slate-400"
                >
                  Cancel
                </Button>
                <Button
                  ...
                  className="rounded-xl h-11 bg-blue-600 hover:bg-blue-700 text-white font-black text-xs uppercase tracking-widest px-10 shadow-lg shadow-blue-500/20"
                >
                  {isSubmitting
                    ? "Saving..."
                    : selectedMeeting
        ...
        <div className="max-w-7xl mx-auto px-6 py-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 mb-10">
            {[
              ...
              {
                label: "Townhall",
                val: meetings.filter((m) => m?.type === "company").length,
                icon: Briefcase,
                bg: "bg-indigo-50",
              ...
              {
                label: "Team Specific",
                val: meetings.filter((m) => m?.type === "team").length,
                icon: Users2,
                bg: "bg-rose-50",
              ...
              },
              {
                ...
                val: meetings.filter((m) => m?.type === "project").length,
                icon: FolderKanban,
                bg: "bg-blue-50",
                darkBg: "dark:bg-blue-900/20",
                text: "text-blue-600",
              ...
              {
                label: "One to One",
                val: meetings.filter((m) => m?.type === "one-to-one").length,
                icon: Users,
                bg: "bg-amber-50",
              ...
              <Card
                key={i}
                className="border-none shadow-sm bg-white dark:bg-slate-900 rounded-[2rem] overflow-hidden hover:shadow-md transition-all duration-300"
              >
                <CardContent className="p-6 flex items-center justify-between">
                  ...
                  <div
                    className={`h-12 w-12 rounded-2xl ${stat.bg} ${stat.darkBg} flex items-center justify-center ${stat.text} ${stat.darkText}`}
                  >
          ...
          <div className="flex items-center gap-2 mb-6">
            ...
            >
              ...
              {myMeetings.length > 0 && (
                <span
                  className={`ml-2 inline-flex items-center justify-center h-5 min-w-[20px] px-1.5 rounded-full text-[9px] font-black ${mainTab === "my" ? "bg-white text-slate-900" : "bg-blue-600 text-white"}`}
                >
                  {myMeetings.length}
                </span>
              )}
          ...
          >
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
              <TabsList className="h-14 p-1.5 rounded-[1.5rem] bg-white dark:bg-slate-900 shadow-sm border-none">
                <TabsTrigger
                  value="all"
                  className="rounded-2xl px-6 font-black text-[10px] uppercase tracking-widest data-[state=active]:bg-slate-900 data-[state=active]:text-white"
                >
                  All
                </TabsTrigger>
                <TabsTrigger
                  value="company"
                  className="rounded-2xl px-6 font-black text-[10px] uppercase tracking-widest data-[state=active]:bg-indigo-600 data-[state=active]:text-white"
                >
                  Townhall
                </TabsTrigger>
                <TabsTrigger
                  value="team"
                  className="rounded-2xl px-6 font-black text-[10px] uppercase tracking-widest data-[state=active]:bg-rose-600 data-[state=active]:text-white"
                >
                  Team
                </TabsTrigger>
                <TabsTrigger
                  value="project"
                  className="rounded-2xl px-6 font-black text-[10px] uppercase tracking-widest data-[state=active]:bg-blue-600 data-[state=active]:text-white"
                >
                  Project
                </TabsTrigger>
                <TabsTrigger
                  value="one-to-one"
                  className="rounded-2xl px-6 font-black text-[10px] uppercase tracking-widest data-[state=active]:bg-amber-500 data-[state=active]:text-white"
                >
                  1:1
                </TabsTrigger>
              </TabsList>
                ...
                {loading ? (
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {[1, 2, 3].map((i) => (
                      <div
                        key={i}
                        className="h-64 rounded-[2.5rem] bg-slate-100 dark:bg-slate-900 animate-pulse border-none"
                      />
                    ))}
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {filteredMeetings.map((meeting) => (
                      <Card
                        key={meeting.id}
                        className="group relative border-none shadow-sm bg-white dark:bg-slate-900 rounded-[2.5rem] overflow-hidden hover:shadow-2xl hover:shadow-blue-500/10 transition-all duration-500"
                      >
                        <div
                          className={`absolute top-0 left-0 right-0 h-1.5 ${getBadgeColor(meeting.type).split(" ")[0]}`}
                        ></div>
                        <CardHeader className="p-8 pb-4">
                          <div className="flex justify-between items-start mb-6">
                            <Badge
                              className={`rounded-full px-4 py-1 font-black text-[9px] uppercase tracking-widest border-none ${getBadgeColor(meeting.type)}`}
                            >
                                ...
                                {(isAdmin ||
                                  ...
                                  <>
                                    <DropdownMenuItem
                                      ...
                                      className="rounded-lg gap-2 font-bold text-xs uppercase tracking-wider py-3"
                                    >
                                    ...
                                    <DropdownMenuItem
                                      ...
                                      className="rounded-lg gap-2 font-bold text-xs uppercase tracking-wider py-3 text-destructive"
                                    >
                          ...
                          <div className="space-y-3 pt-2">
                            <div className="flex items-center gap-3 text-slate-600 dark:text-slate-400">
                              ...
                              <span className="text-xs font-black uppercase tracking-tight">
                                {meeting.start_time &&
                                isValid(new Date(meeting.start_time))
                                  ? format(
                            ...
                            <div className="flex items-center gap-3 text-slate-600 dark:text-slate-400">
                              ...
                              <span className="text-xs font-black tracking-tight">
                                ...
                                -{" "}
                                {meeting.end_time &&
                                isValid(new Date(meeting.end_time))
                                  ? format(
                            ...
                            <div className="flex -space-x-3">
                              {meeting.participants
                                ...
                                .map((p, i) => (
                                  <div
                                    key={i}
                                    className="h-10 w-10 rounded-2xl border-4 border-white dark:border-slate-900 bg-slate-100 flex items-center justify-center overflow-hidden shadow-sm"
                                  >
                                    <img
                                      src={`https://api.dicebear.com/7.x/avataaars/svg?seed=${p.user_name || p.id}`}
                                      alt=""
                                      className="h-full w-full object-cover"
                                    />
                                  </div>
