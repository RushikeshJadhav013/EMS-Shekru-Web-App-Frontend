...
export const OnlineStatusIndicator: React.FC<OnlineStatusIndicatorProps> = ({
  ...
  const loadStatusHistory = async () => {
    ...
    try {
      ...
      // Load status history
      const historyResponse = await fetch(`http://localhost:8000/attendance/online-status/${attendanceId}`, {
        headers: {
      ...
      const hoursResponse = await fetch(`http://localhost:8000/attendance/working-hours/${attendanceId}`, {
        headers: {
          'Authorization': token ? `Bearer ${token}` : '',
        },
      });
